...
tgetglyphs(char *buf, const Glyph *gp, const Glyph *lgp)
{
	while (gp <= lgp)
		if (gp->mode & ATTR_WDUMMY) {
			...
			buf += utf8encode(gp->mode & ATTR_IMAGE ? IMAGE_PLACEHOLDER_CHAR : gp->u, buf);
			gp++;
		}
	return buf;
}
...
{
	if (mode == CURSOR_SAVE)
		term.saved[IS_SET(MODE_ALTSCREEN)] = term.c;
	else if (mode == CURSOR_LOAD) {
		term.c = term.saved[IS_SET(MODE_ALTSCREEN)];
...
tresetcursor(void)
{
	term.c = (TCursor){ { .mode = ATTR_NULL, .fg = defaultfg, .bg = defaultbg, .decor = DECOR_DEFAULT_COLOR },
...
treset(void)
{
	...
	/* Bring both screens to the current dimensions before resetting. */
	if (IS_SET(MODE_ALTSCREEN))
		tloaddefscreen(0, 1);
	tresetcursor();
	tloadaltscreen(0, 0);
...
tsetchar(Rune u, const Glyph *attr, int x, int y)
{
	...
	} else if (x > 0 && (line[x].mode & ATTR_WDUMMY)) {
		line[x-1].u = ' ';
...
draw(void)
{
	...
	if (term.ocx > 0 && (TLINE(term.ocy)[term.ocx].mode & ATTR_WDUMMY))
		term.ocx--;
	if (cx > 0 && (TLINE(term.c.y)[cx].mode & ATTR_WDUMMY))
		cx--;
