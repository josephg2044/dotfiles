...
\begin{note}
    ...
    now, we assume the spectrum is purely discrete.
\end{note}
...
normalized eigenvectors $\ket{E_i}$. These projectors satisfy
...
\begin{proof}
    ...
    \begin{align}
        ...
        &= \delta_{ij} \hat P_i \notag
    \end{align}
    This shows the orthogonality property.
\end{proof}
...
\end{lemma}
\begin{proof}
    Consider an arbitrary state $\ket{\psi}$ in the Hilbert space.
    Let $\hat P_i \ket{\psi} = c \ket{E_i}$ for some scalar $c$, as
...
\begin{proposition}[Expansion of the Projector $\hat P$]
    ...
    \begin{equation} \label{eq:projector_expansion}
        \hat P = \hat P_0+\sum_{n=1}^{\infty}\lambda^n \hat A^{(n)}
    \end{equation}
    where $\hat P_0$ is the unperturbed projector and $\hat
...
\begin{proof}
    ...
    $(E^a_0 - \hat H_0)$ on the $\hat Q_a$ subspace. This means we
    need to show $(E^a_0 - \hat H_0) \hat X = \hat Q_a$ and $\hat X
    (E^a_0 - \hat H_0) = \hat Q_a$.

    First, consider $(E^a_0 - \hat H_0) \hat X$:
    \begin{align} \label{eq:O_a_inverse_proof1}
        (E^a_0 - \hat H_0) \hat X &= (E_a^0 - \hat H_0) \left(
    ...
    \begin{align}
        ...
        &= \hat Q_a \notag
    \end{align}
    Now, consider $\hat X (E^a_0 - \hat H_0)$:
    \begin{align} \label{eq:O_a_inverse_proof2}
        \hat X (E^a_0 - \hat H_0) &=  \left( \sum_{i \ne a}
    ...
    of $(E^a_0 - \hat H_0)$ in the $\hat Q_a$ subspace, which is
...
\end{equation}
\begin{lemma} [Powers of $\hat V \hat G_0$]
    The $nth$ power of $\hat V \hat G_0$ is given by
    \begin{equation}
        (\hat V\hat G_0)^n
        =\sum_{k_1,\ldots,k_n\ge 0}
        \left[\prod_{j=1}^{n}(-1)^{k_j-1}(z-E_0^{\,0})^{k_j-1}\right]
...
\begin{proof}
    ...
    \emph{Base case $n=1$.} This is exactly the displayed formula for
    $\hat V\hat G_0$.

    \emph{Induction step.} Assume $(\ast)$ holds for $n$. Then
    \[
        \begin{aligned}
            (\hat V\hat G_0)^{n+1}
            &=(\hat V\hat G_0)^n(\hat V\hat G_0)\\
            &=\Bigg[\sum_{k_1,\ldots,k_n\ge 0}
                \Big(\prod_{j=1}^{n}(-1)^{k_j-1}(z-E_0^{\,0})^{k_j-1}\Big)
            (\hat V\hat S^{k_1})\cdots(\hat V\hat S^{k_n})\Bigg]
            \Bigg[\sum_{k_{n+1}\ge
                0}(-1)^{k_{n+1}-1}(z-E_0^{\,0})^{k_{n+1}-1}\hat V\hat
            S^{k_{n+1}}\Bigg].
        \end{aligned}
    \]
    Expanding the product of sums (choose one term from each sum,
    preserving operator order) gives
    \[
        (\hat V\hat G_0)^{n+1}
        =\sum_{k_1,\ldots,k_{n+1}\ge 0}
        \Big(\prod_{j=1}^{n+1}(-1)^{k_j-1}(z-E_0^{\,0})^{k_j-1}\Big)
        (\hat V\hat S^{k_1})\cdots(\hat V\hat S^{k_{n+1}}),
    \]
    since the bracketed factors are scalars (functions of $z$) and
    hence commute. This is $(\ast)$ with $n\mapsto n+1$. By
    induction, the identity holds for all $n\ge 1$.
\end{proof}
...
\begin{align} \label{eq:G0_V_G0_n_expansion}
    ...
    S^{k_0}\right] \times \\
    &\quad \left[\sum_{k_1,\cdots,k_n\ge 0}
        (-1)^{\left(\sum_{j=1}^{n}k_j\right)-n}
        (z-E_a^0)^{\left(\sum_{j=1}^{n}k_j\right)-n}
    (\hat V \hat S^{k_1})\cdots(\hat V \hat S^{k_n})\right] \\
    &=\sum_{k_0,\cdots,k_n\ge 0}
...
\begin{equation} \label{eq:A_n_final}
    ...
    \hat S^{k_0} \hat V \hat S^{k_1}\cdots \hat V \hat S^{k_n}
    = - \sum_{(n)} \hat S^{k_1} \hat V \hat S^{k_2}\cdots \hat V \hat
    S^{k_{n+1}},
